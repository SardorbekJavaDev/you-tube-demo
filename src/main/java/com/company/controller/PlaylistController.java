...
public class PlaylistController {
    ...
    @ApiOperation(value = "Create", notes = "Method used for create playlist", authorizations = @Authorization(value = "JWT Token"))
    @PostMapping("public/{channelId}")
    public ResponseEntity<?> create(@RequestBody @Valid PlaylistDTO dto, @PathVariable("channelId") String channelId, HttpServletRequest request) {
        log.info("CREATE {}", dto);
        return ResponseEntity.ok(playlistService.create(dto, channelId, JwtUtil.getIdFromHeader(request)));
    }

    @ApiOperation(value = "Update About", notes = "Method used for update about of playlist", authorizations = @Authorization(value = "JWT Token"))
    @PutMapping("/public/{playlistId}")
    public ResponseEntity<?> updateAbout(@RequestBody @Valid PlaylistAboutDTO dto, @PathVariable("playlistId") String playlistId, HttpServletRequest request) {
    ...
    public ResponseEntity<?> delete(@PathVariable("playlistId") String playlistId, HttpServletRequest request) {
    ...
    @ApiOperation(value = "Change Status", notes = "Method used for change playlist's status with admin or owner", authorizations = @Authorization(value = "JWT Token"))
    @PutMapping("/public/status/{playlistId}")
    public ResponseEntity<?> changeStatus(@PathVariable("playlistId") String playlistId, HttpServletRequest request) {
    ...
    @ApiOperation(value = "List", notes = "Method used for get list of playlists", authorizations = @Authorization(value = "JWT Token"))
    @GetMapping("/adm/list")
    public ResponseEntity<?> list(@RequestParam(value = "page", defaultValue = "0") int page, @RequestParam(value = "size", defaultValue = "5") int size, HttpServletRequest request) {
        ...
        return ResponseEntity.ok(playlistService.list_jpql(page, size));
    }

    @ApiOperation(value = "Profile's Playlist", notes = "Method used for get profile's playlists", authorizations = @Authorization(value = "JWT Token"))
    @GetMapping("/adm/{profileId}/list")
    public ResponseEntity<?> profilePlaylist(@PathVariable("profileId") String profileId, HttpServletRequest request) {
